...
-- SQL Server 2016+ | Optimizadas para Power BI, Vue.js, etc.
-- ============================================================================
...
SELECT
    CAST(o.fecha_orden AS DATE)                                     AS fecha,
    DATENAME(WEEKDAY, o.fecha_orden)                                AS nombre_dia,
    COUNT(*)                                                        AS ordenes,
...
-- Query 4: Performance Top 10 Vendedores (Mes Actual)
SELECT TOP 10
    ...
    ROUND(SUM(o.monto_total * v.tasa_comision / 100.0), 2)                             AS comision_ganada,
    COUNT(DISTINCT o.cliente_id)                                                        AS clientes_unicos,
...
-- Query 5: Comparativa YTD vs Año Anterior
WITH ytd_actual AS (
...
SELECT
    ...
    ROUND(
        100.0 * (COALESCE(a.ventas_ytd,0) - COALESCE(ant.ventas_ytd,0))
        / NULLIF(COALESCE(ant.ventas_ytd,1), 0), 2
...
SELECT
    ...
    c.valor_vida_total,
    MAX(o.fecha_orden)                                      AS fecha_ultima_compra,
    DATEDIFF(DAY, MAX(o.fecha_orden), GETDATE())            AS dias_inactivo,
...
SELECT TOP 20
    ...
    ROUND(SUM(CAST(io.cantidad AS DECIMAL(15,2)) * p.precio_costo), 2)                 AS costo_total,
...
SELECT
    ...
    p.precio_lista,
    p.fecha_lanzamiento,
    DATEDIFF(DAY, p.fecha_lanzamiento, GETDATE())   AS dias_desde_lanzamiento,
    p.stock_actual,
    CASE
        WHEN p.fecha_lanzamiento > DATEADD(DAY, -90, GETDATE()) THEN 'Nuevo'
...
SELECT
    ...
    ROUND(SUM(io.total_linea) - SUM(CAST(io.cantidad AS DECIMAL(15,2)) * p.precio_costo), 2)      AS ganancia_bruta,
    ROUND(100.0 * (SUM(io.total_linea) - SUM(CAST(io.cantidad AS DECIMAL(15,2)) * p.precio_costo))
...
SELECT
    ...
    o.monto_total,
    o.estado_pago,
    DATEDIFF(DAY, o.fecha_orden, GETDATE())                 AS dias_atraso,
    CASE
        WHEN DATEDIFF(DAY, o.fecha_orden, GETDATE()) > 90  THEN 'Crítico'
...
SELECT TOP 10
    ...
    ROUND(100.0 * COUNT(*) / NULLIF((SELECT COUNT(*) FROM dbo.devoluciones), 0), 2)                AS pct_del_total
FROM dbo.devoluciones d
...
SELECT
    ...
    ROUND(100.0 * CAST(COALESCE(c.clics,0) AS DECIMAL) / NULLIF(COALESCE(c.impresiones,1), 0), 2)         AS ctr_pct,
    COALESCE(c.conversiones, 0)                                                                             AS conversiones,
    ROUND(100.0 * CAST(COALESCE(c.conversiones,0) AS DECIMAL) / NULLIF(COALESCE(c.clics,1), 0), 2)        AS tasa_conversion_pct,
    COALESCE(c.ingresos_generados, 0)                                                                       AS ingresos_generados,
    CASE WHEN COALESCE(c.gasto_real,0) > 0
         THEN ROUND(COALESCE(c.ingresos_generados,0) / COALESCE(c.gasto_real,1), 2)
         ELSE 0 END                                                                                         AS roi
FROM dbo.campanas c
WHERE c.estado = 'activa'
   OR c.fecha_fin >= DATEADD(DAY, -30, CAST(GETDATE() AS DATE))
...
-- Query 21: Pronóstico Simple (usando fn_pronostico_ventas)
SELECT mes_pronostico, ingreso_pronosticado
FROM dbo.fn_pronostico_ventas(3, 12)
...
SELECT
    'Semana'                                                                                    AS periodo,
    DATEADD(DAY, -6, CAST(MAX(o.fecha_orden) AS DATE))                                         AS fecha_inicio,
    CAST(MAX(o.fecha_orden) AS DATE)                                                            AS fecha_fin,
    COUNT(*)                                                                                    AS ordenes,
    SUM(o.monto_total)                                                                          AS ingresos,
...
-- ============================================================================
-- LLAMADAS A FUNCIONES ANALÍTICAS
-- ============================================================================
...
-- Tasa de churn
SELECT * FROM dbo.fn_calcular_churn(90)  ORDER BY tasa_churn_pct DESC;
